...
/**
 * bcryptjs may fail to verify hashes generated by other tools using the $2b$ prefix
 * (e.g. Python's passlib, some PHP libs). Normalizing to $2a$ before comparison
 * fixes cross-tool compatibility without affecting security.
 */
function bcryptCompare(password: string, hash: string): boolean {
  const normalizedHash = hash.startsWith('$2b$') ? hash.replace('$2b$', '$2a$') : hash;
...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.post('/auth/login', async (request, reply) => {
    ...
    const passwordOk = user ? bcryptCompare(password, user.password_hash) : false;
  ...
  app.post('/rest/v1/auth/verify-master', async (request, reply) => {
    ...
    const isValid = bcryptCompare(parsed.data.password, masterUser.password_hash);
    if (!isValid) {
  ...
  app.post('/rest/v1/auth/update-password', async (request, reply) => {
    ...
    if (!user || !bcryptCompare(parsed.data.currentPassword, user.password_hash)) {
