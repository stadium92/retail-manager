...
export function SanifereFooter({
  ...
  return (
    <div className="bg-[hsl(220,20%,90%)] dark:bg-[#0D0D0D] border-t-2 border-black/30 dark:border-[#F5C518]/20 font-mono select-none">
      <div className="flex items-center justify-end px-4 py-2 bg-white/80 dark:bg-[#111111] border-b border-black/10 dark:border-white/10">
        <span className="text-lg font-bold text-black dark:text-white mr-4 uppercase">{t('menu.program.netToPay')}</span>
        <span className="text-3xl font-bold text-black bg-[hsl(50,100%,60%)] dark:bg-[#F5C518]/20 dark:text-[#F5C518] px-6 py-1 min-w-[200px] text-right shadow-inner border border-black/20 dark:border-[#F5C518]/30">
          {formatCurrency(netTotal)}
        </span>
      </div>

      <div className="flex items-center gap-1.5 px-2 py-2 bg-[hsl(220,15%,80%)] dark:bg-[#0D0D0D] overflow-x-auto shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] dark:shadow-none">
        {functionKeys.map((fk) => (
          <button
            key={fk.key}
            onClick={() => handleAction(fk.action)}
            className={cn(
              "relative flex flex-col items-center justify-center min-w-[60px] h-[50px] px-2 cursor-pointer",
              "bg-gradient-to-b from-[#f0f0f0] to-[#d0d0d0] dark:from-[#222222] dark:to-[#111111]",
              "border border-b-2 border-r-2 border-white/50 border-b-black/40 border-r-black/40 dark:border-white/10 dark:border-b-black/80 dark:border-r-black/80",
              "rounded-sm text-xs font-bold text-black dark:text-white shadow-sm transition-all active:translate-y-0.5 active:shadow-none",
              isActive(fk.key) && "bg-gradient-to-b from-[#e0e0e0] to-[#c0c0c0] dark:from-[#333333] dark:to-[#222222] translate-y-0.5 shadow-none border-t-black/20 border-l-black/20 dark:border-t-black/50 dark:border-l-black/50"
            )}
          >
            <span className="text-[hsl(0,70%,45%)] dark:text-[#F5C518] mb-0.5 text-sm">{fk.key}</span>
            <span className="uppercase tracking-tight leading-none text-[10px] opacity-90">{fk.label}</span>
          </button>
        ))}
      </div>
    </div>
