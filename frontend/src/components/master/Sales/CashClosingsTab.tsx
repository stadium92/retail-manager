            ...
            <TableBody>
              {loading && closings.length === 0 ? (
                ...
                <TableRow key={c.id} className="border-b-2 hover:bg-muted/30 h-16 group transition-colors">
                  ...
                  <TableCell className="text-[10px] font-black uppercase">{workerMap[c.worker_id] || ((loading && !workerMap[c.worker_id]) ? '...' : (c.worker_id ? (String(c.worker_id).length < 15 ? String(c.worker_id) : String(c.worker_id).slice(0,8)) : '?'))}</TableCell>
