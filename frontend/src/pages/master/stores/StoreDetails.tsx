...
export default function StoreDetailsPage() {
  ...
  const loadStoreDetails = async (storeId: string) => {
    ...
    try {
      // Fetch store
      const { data: store, error: storeError } = await StoreService.getStore(storeId);
      if (storeError || !store) {
        toast({
      ...
      // Fetch inventory
      const { data: inventory } = await InventoryService.getInventory(storeId);

      // Fetch recent sales
      const { data: recentSales } = await SalesService.getSales(storeId, 10);

      // Calculate metrics
      const metrics = await SalesService.getSalesMetrics(storeId);
