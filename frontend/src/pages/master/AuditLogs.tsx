...
import { ScrollText, Search, User, Loader2, AlertCircle, Database, Wrench, Trash2 } from 'lucide-react';
...
export default function AuditLogsPage() {
  ...
  const handleRepairDb = async () => {
    try {
      ...
      const res = await fetch(`${localBridgeBaseUrl}/system-repair`, {
        method: 'POST',
        headers: { ...headers }
      });
  ...
  const handleHardReset = async () => {
    ...
    try {
      if (!isLocalFirst) return;
      const headers = await OfflineAuthService.getAuthHeaders();
      const res = await fetch(`${localBridgeBaseUrl}/system-hard-reset`, {
        method: 'POST',
        headers: { ...headers }
      });
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="destructive" size="sm" onClick={handleRepairDb}>
            <Wrench className="h-4 w-4 mr-2" />
            {t('audit.repairDb')}
          </Button>
          <Button variant="outline" size="sm" className="border-red-500 text-red-500 hover:bg-red-50" onClick={handleHardReset}>
