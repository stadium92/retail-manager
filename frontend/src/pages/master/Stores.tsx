...
export default function StoresPage() {
    ...
    setLoading(false);
      ...
      const handleOpenForm = (store?: Store) => {
        if (!store) {
          ...
          setFormData({ name: '', address: '', phone: '', image_url: '' });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedStore) {
      ...
      const { error } = await StoreService.updateStore(selectedStore.id, updates);
      if (error) {
        toast({
          title: t('common.error'),
      ...
      const { error } = await StoreService.createStore(storeData);
      if (error) {
  ...
  const handleDelete = async () => {
    ...
    const { error } = await StoreService.deleteStore(selectedStore.id);
    if (error) {
      toast({
        title: t('common.error'),
