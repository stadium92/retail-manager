...
import { TeamMember, CreateUserPayload } from './TeamService';
...
export class OfflineTeamService {
  ...
  static async createUser(
    ...
    try {
      ...
      if (!isOnline) {
        ...
        toast({
          title: `${payload.role === 'worker' ? 'Worker' : 'Deliverer'} Created (Offline)`,
      ...
      if (!provisionResult.success) {
        ...
        toast({
          title: `${payload.role === 'worker' ? 'Worker' : 'Deliverer'} Created Locally`,
      ...
      toast({
        title: `${payload.role === 'worker' ? 'Worker' : 'Deliverer'} Created`,
        description: 'Created and synced successfully.',
      });
  ...
  static async updateUserStatus(userId: string, isActive: boolean): Promise<{ success: boolean; error?: any }> {
    try {
      ...
      if (!navigator.onLine) {
        ...
        toast({
          title: isActive ? 'Account Enabled (Offline)' : 'Account Disabled (Offline)',
  ...
  private static async getDeliverersViaLocalBridge(): Promise<{ data?: TeamMember[]; error?: any }> {
    try {
      ...
      const [rolesRes, usersRes, statsRes] = await Promise.all([
        fetch(`${baseUrl}/rest/v1/user_roles?role=deliverer`, { headers }),
        fetch(`${baseUrl}/rest/v1/users`, { headers }),
        fetch(`${baseUrl}/rest/v1/deliverer_stats`, { headers }),
      ]);
  ...
  private static async getWorkersViaLocalBridge(): Promise<{ data?: TeamMember[]; error?: any }> {
    try {
      ...
      const [rolesRes, usersRes, storesRes, salesRes] = await Promise.all([
        fetch(`${baseUrl}/rest/v1/user_roles?role=worker`, { headers }),
        fetch(`${baseUrl}/rest/v1/users`, { headers }),
        fetch(`${baseUrl}/rest/v1/stores`, { headers }),
        fetch(`${baseUrl}/rest/v1/sales`, { headers }),
      ]);
  ...
  private static async createUserViaLocalBridge(
    ...
    try {
      ...
      toast({
        title: `${payload.role === 'worker' ? 'Worker' : 'Deliverer'} Created`,
        description: 'Stored in the local backend.',
      });
  ...
  static async getStores(): Promise<{ data?: any[]; error?: any }> {
    try {
      ...
      if (dataClient.isLocalFirst) {
        ...
        const response = await fetch(`${dataClient.localBridgeBaseUrl}/rest/v1/stores`, { headers });
