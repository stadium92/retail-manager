          ...
          {detailsLoading ? (
            ...
            <>
              ...
              <section className="grid grid-cols-2 gap-3">
                <div className="bg-[#141414] border border-rs-surface-container-highest p-4 rounded-2xl flex flex-col gap-1">
                  <TrendingUp className="w-5 h-5 text-emerald-400 mb-1" />
                  <span className="text-xs text-rs-on-surface-variant">Ventes Aujourd'hui</span>
                ...
                <div className="bg-[#141414] border border-rs-surface-container-highest p-4 rounded-2xl flex flex-col gap-1">
                  ...
                  <span className="text-xs text-rs-on-surface-variant">Menu / Produits</span>
                  <span className="text-lg font-bold font-mono text-white">
                    {storeDetails.inventoryCount} articles
                  </span>
                </div>
              </section>
              ...
              {storeDetails.lowStockCount > 0 && (
                <div className="bg-red-950/20 border border-red-800/40 p-4 rounded-2xl flex items-center gap-3 text-red-200">
                  <ShieldAlert className="w-6 h-6 text-red-400 shrink-0" />
                  <div className="text-sm">
                    <p className="font-bold">{storeDetails.lowStockCount} Articles en Alerte Stock</p>
                    <p className="text-xs text-red-300/80">Le niveau est inférieur au seuil d'alerte.</p>
                  </div>
                </div>
              )}
              ...
              {/* Personnel / Workers */}
              <section className="bg-[#141414] border border-rs-surface-container-highest rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between border-b border-rs-surface-container pb-2">
                  <h3 className="text-sm font-bold text-rs-surface-tint uppercase tracking-wider flex items-center gap-2">
                    <Users className="w-4 h-4" />
                    Personnel ({storeDetails.workers.length})
                  </h3>
                </div>
                {storeDetails.workers.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-[200px] overflow-y-auto">
                    {storeDetails.workers.map((worker: any) => (
                      <div key={worker.id} className="flex justify-between items-center p-2 rounded-lg bg-rs-surface-container-lowest border border-rs-surface-container">
                        ...
                        <span className="text-[10px] uppercase bg-rs-surface-tint/25 text-rs-surface-tint px-2 py-0.5 rounded-full font-bold">
          ...
          <div className="flex flex-col gap-3">
            {stores.map(store => (
              ...
              >
                ...
                <div className="flex items-center gap-1 text-rs-on-surface-variant">
                  <span className="text-xs uppercase bg-rs-surface-tint/15 text-rs-surface-tint font-semibold px-2 py-0.5 rounded-full">
