...
export default function StoreDetailsPage() {
  ...
  const loadStoreDetails = async (storeId: string) => {
    ...
    try {
      ...
      // Fetch workers
      const { data: allWorkersRes } = await OfflineTeamService.getWorkers();
      const workers = (allWorkersRes || [])
        .filter(w => w.store_id === storeId)
        .map(w => ({
          profile: {
            id: w.user_id,
          ...
          role: w.sub_role 
            ? `${w.role === 'worker' ? 'Personnel' : w.role} (${w.sub_role === 'cook' ? 'Cuisinier' : w.sub_role === 'cashier' ? 'Caissier' : w.sub_role === 'waiter' ? 'Serveur' : w.sub_role})` 
            : (w.role === 'worker' ? 'Personnel' : w.role),
        }));
