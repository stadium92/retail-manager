      ...
      try {
        ...
        if (path.startsWith('/rest/v1/sales')) {
          ...
          if (init.method === 'POST') {
            ...
            const saleId = body.id || crypto.randomUUID();
            ...
            if (items && items.length > 0) {
              ...
              const { error: itemsErr } = await supabase.from('sale_items').insert(mappedItems);
              if (itemsErr) throw itemsErr;
            }
            return { id: saleId, ...saleData } as any;
          } else if (path.includes('/rest/v1/sales/')) {
            const parts = path.split('/');
            const saleId = parts[4] ? parts[4].split('?')[0] : null;
            if (init.method === 'PATCH' && saleId) {
              const body = JSON.parse(init.body as string);
        ...
        // Intercept cashier credits
        if (path.startsWith('/rest/v1/cashier_credits')) {
          const parts = path.split('/');
          const id = parts[4] ? parts[4].split('?')[0] : null;
        ...
        // Intercept stock adjustments
        if (path.startsWith('/rest/v1/stock_adjustments')) {
          if (init.method === 'POST') {
            const body = JSON.parse(init.body as string);
            const mapped = {
              id: body.id || crypto.randomUUID(),
              ...
              quantity_adjusted: Number(body.quantity_adjusted),
        ...
        // Intercept stock valuation
        if (path.includes('/analytics/stock-valuation')) {
          const urlObj = new URL(`http://localhost${path}`);
          const storeId = urlObj.searchParams.get('store_id');
          
          let query = supabase.from('products').select('cost_price, unit_price, quantity');
          if (storeId) {
            query = query.eq('store_id', storeId);
          }
