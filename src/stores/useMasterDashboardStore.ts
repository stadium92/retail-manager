  ...
  persist(
    (set) => ({
      ...
      setSelectedStoreIds: (ids) => set((state) => ({ 
        ...
        isAllStoresSelected: ids.length === 0 ? true : false,
      ...
      toggleStoreSelection: (id) => set((state) => {
        ...
        const shouldSelectAll = newIds.length === 0;

        return { 
          selectedStoreIds: shouldSelectAll ? [] : newIds,
          isAllStoresSelected: shouldSelectAll,
          version: state.version + 1
        };
      }),

      setAllStoresSelected: (selected, allIds) => set((state) => ({
        selectedStoreIds: selected ? [] : (allIds.length > 0 ? [allIds[0]] : []),
