...
Write-Host "🔧 Recompiling better-sqlite3 for 32-bit Windows..." -ForegroundColor Yellow
npm rebuild better-sqlite3 --arch=$TARGET_ARCH

# Find the binary
$BINARY_SOURCE = "node_modules/better-sqlite3/build/Release/better_sqlite3.node"
if (!(Test-Path $BINARY_SOURCE)) {
    Write-Error "❌ Error: Native module build failed. File not found at $BINARY_SOURCE"
}

# Copy the binary
Write-Host "📦 Copying native binary..."
Copy-Item $BINARY_SOURCE "../../$TAURI_BIN_DIR/better_sqlite3.node" -Force
...
Write-Host "⚠️  Note: Your local better-sqlite3 is now 32-bit. Run 'npm rebuild' to restore 64-bit dev mode if needed."
